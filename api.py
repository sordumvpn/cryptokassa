...
BOT_TOKEN = "8744785117:AAGhQuyvVW5WAqqEJBoRoK7JI5kUpyq1NTQ"  # Вставь токен из @BotFather

app = FastAPI(title="CryptoKassa API")

# Настройка CORS для взаимодействия с GitHub Pages
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Автоматический обход экрана предупреждения ngrok
@app.middleware("http")
...
def verify_telegram_data(init_data: str) -> dict:
    if not init_data:
        print("--> WARNING: initData отсутствует, используется fallback-юзер")
    ...
    try:
        ...
        if calculated_hash != hash_from_tg:
            print("--> WARNING: Хэш Telegram не совпал с BOT_TOKEN.")
...
async def process_withdraw(req: WithdrawRequest, x_init_data: str = Header(None)):
    tg_user = verify_telegram_data(x_init_data or "")
    user_id = tg_user.get("id")
    ...
    col = coin_column_map.get(req.coin.upper())
    if not col:
    ...
    try:
        ...
        if not row or float(row[0] or 0.0) < req.amount:
            conn.close()
        ...
        print(
            f"--> УСПЕШНЫЙ ВЫВОД: Игрок {user_id} перевел {req.amount} {req.coin} на {req.address}"
        )
        return {"status": "success", "amount": req.amount, "coin": req.coin}

    except HTTPException:
        raise
    except Exception as e:
        print(f"--> ERROR вывода: {e}")
